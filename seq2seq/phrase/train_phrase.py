...
hidden_size = 256
num_layers = 2
num_encoder_symbols = data_phrase.VOCAB_SIZE + 4  # 'UNK' and '<go>' and '<eos>' and '<pad>'
num_decoder_symbols = data_phrase.VOCAB_SIZE + 4
embedding_size = 256
learning_rate = 0.001
model_dir = './model_phrase'
...
def loadQA():
    train_x = np.load('./idx_q_phra.npy', mmap_mode='r')
    train_y = np.load('./idx_a_phra.npy', mmap_mode='r')
    train_target = np.load('./idx_o_phra.npy', mmap_mode='r')
